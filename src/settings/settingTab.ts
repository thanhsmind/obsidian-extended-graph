...
export class ExtendedGraphSettingTab extends PluginSettingTab {
	...
	override display(): void {
		this.originalSettings = structuredClone(
			ExtendedGraphInstances.settings
		);
	...
	private addConfig(): void {
		this.containerEl.createEl("h3", { text: "Link Label Click Feature" });

		new Setting(this.containerEl)
			.setName("Relation note folder")
			.setDesc(
				"The folder where new relation notes will be created when clicking a link label."
			)
			.addText((text) =>
				text
					.setPlaceholder("Example: Relations")
					.setValue(
						ExtendedGraphInstances.settings.relationFolderPath
					) // <--- ĐÃ SỬA
	...
	private addImportExport(): void {
		new Setting(this.containerEl)
			.addExtraButton((cb) => {
				cb.extraSettingsEl.insertAdjacentText(
					"beforebegin",
					t("controls.export")
				);
				cb.setIcon("upload");
				...
				cb.onClick(() => {
					const modal = new ExportConfigModal(
						(name: string, fullpath: boolean) => {
							const filepath = fullpath
								? name
								: ExtendedGraphInstances.configurationDirectory +
								  "/" +
								  name +
								  ".json";
							if (!fullpath) {
							...
							ExtendedGraphInstances.plugin.exportSettings(
								filepath,
								ExtendedGraphInstances.settings
							);
							return true;
	...
	private addNav(): void {
		const nav = this.containerEl.createDiv({
			cls: "extended-graph-nav-settings",
		});
		...
		if (getDataviewPlugin(false)) {
			new Setting(this.containerEl)
				...
				.addToggle((cb) => {
					...
					cb.onChange((value) => {
						ExtendedGraphInstances.settings.canonicalizePropertiesWithDataview =
							value;
						ExtendedGraphInstances.plugin.saveSettings();
					});
				});
	...
	override hide(): void {
		if (
			ExtendedGraphInstances.graphsManager &&
			ExtendedGraphInstances.settings.resetAfterChanges
		) {
			if (
				SettingQuery.needReload(
			...
			if (
				...
				ExtendedGraphInstances.graphsManager.resetAllPlugins(
					"localgraph"
				);
			}
		}
